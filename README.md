...
- **Three lanes**: `main` (your big local model), `executor` (a small fast
  local model for routine tool calls), and `vision`/`embedder` helpers —
  each can be served locally via `llama-server` or bound to a cloud
  OpenAI-compatible provider, in any mix (`all-local`,
  `main-local-rest-cloud`, `main-cloud-rest-local`, `all-cloud`), with
...
7. **(Optional) Cloud providers** — for hybrid/cloud lanes, add provider
   ...
   holds API keys — see the security note below). Never commit real API
...
- `config/app.json` — main app config (models dir, small-model lanes,
  ...
  delete to reset a model to defaults. `config/providers.json` —
...
(permission-gated but still powerful) and stores API keys in
...
tokens; treat `config/providers.json` and `usage.db` as sensitive data at
rest (not encrypted); and never paste real card numbers, tokens, or merchant
